...
param(
  ...
  [string]$ProjectPath = "C:\PowerBI\FFLEXACHA",
  [switch]$CreateRemote,
  [switch]$Push
)
$ErrorActionPreference = "Stop"
$RepoName = "FFLEXACHA-powerbi-visual"
$ExpectedName = "FFLEXACHA"
...
if (-not (git diff --cached --quiet)) { git commit -m "Initial FFLEXACHA Power BI visual" }
...
if ($CreateRemote) {
  ...
  if ($LASTEXITCODE -ne 0) { gh repo create "$Owner/$RepoName" --public --description "Independent FFLEXACHA Power BI custom visual" --source . --remote origin --push }
...
if ($Push) {
  ...
  if ($PSCmdlet.ShouldProcess("origin/main", "Push FFLEXACHA")) { git push -u origin main }
}
Write-Host "Prepared FFLEXACHA. Remote creation/push are opt-in switches." -ForegroundColor Green
